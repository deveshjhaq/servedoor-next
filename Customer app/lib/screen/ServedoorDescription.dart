...
import 'package:servedoor_customer_app/utils/ServedoorImages.dart';
...
import 'ServedoorBookCart.dart';

class FoodDescription extends StatefulWidget {
  static String tag = '/FoodDescription';
  final String? restaurantId;

  const FoodDescription({super.key, this.restaurantId});
                          ...
                          children: <Widget>[
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: <Widget>[
                                  Text(food_lbl_delivery_by_servedoor_with_online_tracking, style: boldTextStyle()),
  ...
  Future<void> _orderNow() async {
    ...
    try {
      ...
      if (widget.restaurantId != null && widget.restaurantId!.isNotEmpty) {
        ...
        final details = await ServedoorApi.getRestaurantDetails(restaurantId);
        restaurantName = (details['name'] ?? restaurantName).toString();
      } else {
        final restaurants = await ServedoorApi.getRestaurants(page: 1, limit: 1);
        if (restaurants.isEmpty) {
          toast('No restaurants available');
          return;
        }
      ...
      final menu = await ServedoorApi.getRestaurantMenu(restaurantId);
      if (menu.isEmpty) {
        toast('No menu items available');
        return;
      }

      final firstItem = (menu.first as Map).cast<String, dynamic>();
      final menuItemId = (firstItem['id'] ?? '').toString();
      final menuItemName = (firstItem['name'] ?? 'Item').toString();
      final price = (firstItem['price'] as num?)?.toDouble() ?? 0.0;
      final isVeg = firstItem['isVeg'] == true;

      await ServedoorApi.addToCart(
...
class CustomDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: const Text('Live menu is available on the restaurant detail and order flow screens.'),
      actions: [
        TextButton(
