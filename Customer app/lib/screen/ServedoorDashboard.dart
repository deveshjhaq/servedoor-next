...
import 'ServedoorAddAddress.dart';
import 'ServedoorBookCart.dart';
import 'ServedoorDescription.dart';
import 'ServedoorFavourite.dart';
import 'ServedoorOrder.dart';
import 'ServedoorProfile.dart';
import 'ServedoorSignIn.dart';
import 'ServedoorViewRestaurants.dart';
...
class FoodDashboardState extends State<FoodDashboard> {
  ...
  void initState() {
    ...
    _loadDashboardRestaurants();
  }

  Future<void> _loadDashboardRestaurants() async {
    setState(() {
    ...
    try {
      final restaurants = await ServedoorApi.getRestaurants(page: 1, limit: 20);
      if (!mounted || restaurants.isEmpty) return;

      final mapped = restaurants
          .map(
            (r) => Restaurants(
              r.name,
              r.rating,
              r.image,
              '${r.rating.toStringAsFixed(1)} rating',
            ),
          )
          .toList();

      setState(() {
        mDeliveryList = mapped;
        mBakeryList = mapped;
        mDineOutList = mapped;
        mCafeList = mapped;
                    ...
                    Container(
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          if (_restaurantLoadError.isNotEmpty)
                            Row(
                              children: [
                                Expanded(
                                  child: Text(
                                    'Could not sync restaurants from backend',
                                    style: primaryTextStyle(color: food_color_red),
                                  ),
                                ),
                                TextButton(
                                  onPressed: _loadDashboardRestaurants,
